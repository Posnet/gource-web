...
std::string MercurialLog::logCommand() {
    ...
    // date range
    if(!gGourceSettings.start_date.empty() && !gGourceSettings.stop_date.empty()) {
        range = "--date '" + gGourceSettings.start_date + " to " + gGourceSettings.stop_date + "'";
    }
    // start date only
    else if(!gGourceSettings.start_date.empty()) {
        range = "--date '>" + gGourceSettings.start_date + "'";
    }
    // stop date only
    else if(!gGourceSettings.stop_date.empty()) {
        range = "--date '<" + gGourceSettings.stop_date + "'";
    }

    std::string log_command = "hg log " + range + " -r 0:tip --style '" + gource_style_path + "'";
