...
// AGENT: PURPOSE    — Shell wrapper for Gource visualization
...
void GourceShell::blendLastFrame(float dt) {
    ...
    auto& r = renderer();
    r.color(1.0f, 1.0f, 1.0f, transition_interval);
    ...
    r.begin(GL_QUADS);
        ...
        r.vertex((float)display.width, 0.0f);

        r.texcoord(1.0f, 0.0f);
        r.vertex((float)display.width, (float)display.height);

        r.texcoord(0.0f, 0.0f);
        r.vertex(0.0f, (float)display.height);
    r.end();
