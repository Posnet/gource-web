// === File: src/textbox.cpp ===================================================
// AGENT: PURPOSE    — Text box UI widget for tooltips
...
void TextBox::draw() const {
    ...
    r.color(0.0f, 0.0f, 0.0f, alpha * 0.333f);

    r.pushModelView();
    r.translateMV(shadow.x, shadow.y, 0.0f);

    r.begin(GL_QUADS);
        r.vertex(corner.x, corner.y);
        r.vertex(corner.x, corner.y + rect_height);
        r.vertex(corner.x + rect_width, corner.y + rect_height);
        r.vertex(corner.x + rect_width, corner.y);
    r.end();

    r.popModelView();

    r.color(colour.x * brightness, colour.y * brightness, colour.z * brightness, alpha);
