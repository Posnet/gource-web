// === File: src/gource.cpp ====================================================
// AGENT: PURPOSE    — Main Gource visualization class
...
Gource::Gource(FrameExporter* exporter) {
    ...
    // WebGL always supports OpenGL ES 2.0+, so always use modern path
    gGourceSettings.ffp = false;
    ...
    std::string bloom_tga = gGourceSettings.transparent ? "bloom_alpha.png" : "bloom.png";
    ...
    if(gGourceSettings.logo.size() > 0) {
        // WebGL always supports non-power-of-two textures
        logotex = texturemanager.grabFile(gGourceSettings.logo, false);
    }
...
void Gource::keyPress(SDL_KeyboardEvent *e) {
    ...
    if (e->type == SDL_KEYDOWN) {
        ...
        if (e->keysym.sym == SDLK_p) {
            // Toggle FFP mode (always available in WebGL with shaders)
            if(bloom_shader != 0) {
...
void Gource::drawBackground(float dt) {
    ...
    if(backgroundtex!=0) {
        ...
        r.bindTexture(backgroundtex->textureid);

        r.pushModelView();
        r.translateMV(display.width/2.0f - backgroundtex->w/2.0f, display.height/2.0f - backgroundtex->h/2.0f, 0.0f);
        ...
        r.begin(GL_QUADS);
            ...
            r.vertex((float)backgroundtex->w, 0.0f);

            r.texcoord(1.0f, 1.0f);
            r.vertex((float)backgroundtex->w, (float)backgroundtex->h);

            r.texcoord(0.0f, 1.0f);
            r.vertex(0.0f, (float)backgroundtex->h);
        r.end();
...
void Gource::updateAndDrawEdges() {
    ...
    // Switch to 2D mode
    r.push2D(display.width, display.height);
    ...
    if(!gGourceSettings.ffp) {
        ...
        shadow_shader->setSampler2D("u_texture", 0);
        shadow_shader->setFloat("u_shadow_strength", 0.5);
        shadow_shader->use();

        vec2 shadow_offset = vec2(2.0, 2.0);

        r.pushModelView();
        r.translateMV(shadow_offset.x, shadow_offset.y, 0.0f);

        // Set MVP matrix for shadow shader
        GLint mvp_loc = shadow_shader->getUniformLocation("u_mvp");
        if (mvp_loc >= 0) {
            glm::mat4 mvp = r.getMVP();
            glUniformMatrix4fv(mvp_loc, 1, GL_FALSE, glm::value_ptr(mvp));
        }

        edge_vbo.draw(false);  // Use external shadow_shader
        r.popModelView();

        glUseProgram(0);

        edge_vbo.draw(true);  // Use basic_shader for main edges
...
void Gource::drawActions(float dt) {
    ...
    if(!gGourceSettings.ffp) {
        ...
        action_vbo.draw(true);  // Use basic_shader for actions
...
void Gource::drawBloom(float dt) {
    ...
    if(!gGourceSettings.ffp) {
        ...
        // Set MVP matrix for bloom shader
        auto& r = renderer();
        GLint mvp_loc = bloom_shader->getUniformLocation("u_mvp");
        if (mvp_loc >= 0) {
            glm::mat4 mvp = r.getMVP();
            glUniformMatrix4fv(mvp_loc, 1, GL_FALSE, glm::value_ptr(mvp));
        }
...
void Gource::drawFileShadows(float dt) {
    ...
    if(!gGourceSettings.ffp) {
        ...
        file_vbo.draw(false);  // Use external shadow_shader
        r.popModelView();
...
void Gource::drawUserShadows(float dt) {
    ...
    if(!gGourceSettings.ffp) {
        ...
        user_vbo.draw(false);  // Use external shadow_shader
...
void Gource::draw(float t, float dt) {
    ...
    // Get matrices from renderer (convert to double for compatibility)
    const glm::mat4& mv = r.getModelView();
    const glm::mat4& proj = r.getProjection();
    for (int i = 0; i < 16; i++) {
        modelview[i] = (GLdouble)mv[i / 4][i % 4];
        projection[i] = (GLdouble)proj[i / 4][i % 4];
    }
   ...
   if(!(gGourceSettings.hide_usernames || gGourceSettings.hide_users)) {
    ...
    if(!gGourceSettings.ffp) {
        ...
        text_shader->setSampler2D("u_texture", 0);
        text_shader->setFloat("u_shadow_strength", 0.7);
        text_shader->setFloat("u_texel_size", font_texel_size);
        text_shader->use();

        // Set MVP matrix for text shader
        GLint mvp_loc = text_shader->getUniformLocation("u_mvp");
        if (mvp_loc >= 0) {
    ...
    if(gGourceQuadTreeDebug) {
        ...
        r.color(0.0f, 1.0f, 1.0f, 1.0f);

        userTree->outline();

        r.color(0.0f, 1.0f, 0.5f, 1.0f);
    ...
    if(logotex!=0) {
        ...
        r.begin(GL_QUADS);
            ...
            r.texcoord(1.0f, 0.0f);
            r.vertex((float)logotex->w, 0.0f);

            r.texcoord(1.0f, 1.0f);
            r.vertex((float)logotex->w, (float)logotex->h);

            r.texcoord(0.0f, 1.0f);
            r.vertex(0.0f, (float)logotex->h);
        r.end();
    ...
    if(splash>0.0f) {
        ...
        glDisable(GL_TEXTURE_2D);
        r.unbindTexture();
        r.color(0.0f, 0.5f, 1.0f, splash * 0.015f);
        r.begin(GL_QUADS);
            r.vertex(0.0f, corner.y);
            r.vertex(0.0f, corner.y + logoheight);
            r.vertex((float)display.width, corner.y + logoheight);
            r.vertex((float)display.width, corner.y);
        r.end();
