...
// Simple string formatting helper to replace boost::format
...
Regex Shader_pre_version("^\\s*#version\\s*(\\d+)\\s*(es)?\\s*");
...
void ShaderPart::loadSourceFile() {
    ...
    if(!in.is_open()) {
        throw ShaderException(fmt("could not open '%s'", filename.c_str()));
    }
...
void FloatShaderUniform::write(std::string& content) const {
    if(baked) {
        content.append(fmt("#define %s %e\n", name.c_str(), value));
    } else {
        content.append(fmt("uniform %s %s;\n", type_name.c_str(), name.c_str()));
    }
}
...
void Vec2ShaderUniform::write(std::string& content) const {
    if(baked) {
        content.append(fmt("#define %s vec2(%e, %e)\n", name.c_str(), value.x, value.y));
    } else {
        content.append(fmt("uniform %s %s;\n", type_name.c_str(), name.c_str()));
    }
}
...
void Vec3ShaderUniform::write(std::string& content) const {
    if(baked) {
        content.append(fmt("#define %s vec3(%e, %e, %e)\n", name.c_str(), value.x, value.y, value.z));
    } else {
        content.append(fmt("uniform %s %s;\n", type_name.c_str(), name.c_str()));
    }
}
...
void Mat3ShaderUniform::write(std::string& content) const {
    if(baked) {
        content.append(fmt("#define %s mat3(%e, %e, %e, %e, %e, %e, %e, %e, %e)\n",
               name.c_str(),
...
void Mat4ShaderUniform::write(std::string& content) const {
    if(baked) {
        content.append(fmt("#define %s mat4(%e, %e, %e, %e, %e, %e, %e, %e, %e, %e, %e, %e, %e, %e, %e, %e)\n",
               name.c_str(),
...
void IntegerArrayShaderUniform::write(std::string& content) const {
    if(baked) {
        content.append(fmt("%s[%d] %s = %s[] (\n", type_name.c_str(), (int)length, name.c_str(), type_name.c_str()));

        for(size_t i=0; i<length; i++) {
            content.append(fmt("    %d", value[i]));
        ...
        content.append(fmt("uniform %s %s[%d];\n", type_name.c_str(), name.c_str(), (int)length));
    }
}
...
void Vec2ArrayShaderUniform::write(std::string& content) const {
    ...
    if(baked) {
        ...
        for(size_t i=0; i<length; i++) {
            content.append(fmt("    %s(%e, %e)", type_name.c_str(), value[i].x, value[i].y));
...
void Vec3ArrayShaderUniform::write(std::string& content) const {
    ...
    if(baked) {
        ...
        for(size_t i=0; i<length; i++) {
            content.append(fmt("    %s(%e, %e, %e)", type_name.c_str(), value[i].x, value[i].y, value[i].z));
...
void Vec4ArrayShaderUniform::write(std::string& content) const {
    if(baked) {
        ...
        for(size_t i=0; i<length; i++) {
            content.append(fmt("    %s(%e, %e, %e, %e)", type_name.c_str(), value[i].x, value[i].y, value[i].z, value[i].w));
...
void AbstractShaderPass::toString(std::string& out) {
    if(version!=0) {
        if(version_es) {
            out.append(fmt("#version %d es\n", version));
            // For GLSL ES, precision must be declared before any float uniforms
            out.append("precision highp float;\n");
        } else {
            out.append(fmt("#version %d\n", version));
        }
    }

    for(std::map<std::string, std::string>::iterator it = extensions.begin(); it != extensions.end(); it++) {
        out.append(fmt("#extension %s : %s\n", it->first.c_str(), it->second.c_str()));
    }
...
ShaderUniform* AbstractShaderPass::addArrayUniform(const std::string& name, const std::string& type, size_t length) {
    ...
    if((uniform = parent->getUniform(name)) == 0) {
        ...
        if(type == "int") {
            ...
            throw ShaderException(fmt("shader uniform arrays for type '%s' not implemented", type.c_str()));
        }
...
ShaderUniform* AbstractShaderPass::addUniform(const std::string& name, const std::string& type) {
    ...
    if((uniform = parent->getUniform(name)) == 0) {
        ...
        if(type == "float") {
            ...
            throw ShaderException(fmt("unsupported shader uniform type '%s'", type.c_str()));
        }
...
bool AbstractShaderPass::preprocess(const std::string& line) {
    ...
    if(Shader_pre_version.match(line, &matches)) {
        ...
        version_es = (matches.size() > 1 && matches[1] == "es");
        return true;
    }
...
void AbstractShader::addUniform(ShaderUniform* uniform) {
    ...
    if(getUniform(uniform->getName()) != 0) {
        throw ShaderException(fmt("shader already has a uniform named '%s'", uniform->getName().c_str()));
    }
