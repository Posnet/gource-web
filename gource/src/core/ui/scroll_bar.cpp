// === File: src/core/ui/scroll_bar.cpp =========================================
// AGENT: PURPOSE    — Scrollbar UI control
...
void UIScrollBar::drawContent() {
    ...
    if(horizontal) {
        r.translateMV(bar_visual_offset * rect.x, 0.0f, 0.0f);
    } else {
        r.translateMV(0.0f, bar_visual_offset * rect.y, 0.0f);
    }
    ...
    for(int i=0;i<4;i++) {
        ...
        if(inverted) {
            ...
            switch(i) {
                ...
                case 1:
                    ...
                    r.translateMV(bar_rect.x*0.5f, 0.0f, 0.0f);
                    break;
                case 2:
                    texcoord = vec4(bar_rect.x/32.0f, bar_rect.y/32.0f, 0.0f, 0.0f);
                    r.translateMV(bar_rect.x*0.5f, bar_rect.y*0.5f, 0.0f);
                    break;
                case 3:
                    texcoord = vec4(1.0f, (bar_rect.y/32.0f), 1.0f-(bar_rect.x/32.0f), 0.0f);
                    r.translateMV(0.0f, bar_rect.y*0.5f, 0.0f);
                    break;
            }
        } else {
            bartex[i]->bind();
        ...
        r.begin(GL_QUADS);
            ...
            r.vertex(bar_rect.x*0.5f, 0.0f);

            r.texcoord(texcoord.z, texcoord.w);
            r.vertex(bar_rect.x*0.5f, bar_rect.y*0.5f);

            r.texcoord(texcoord.x, texcoord.w);
            r.vertex(0.0f, bar_rect.y*0.5f);
        r.end();
