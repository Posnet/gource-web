// === File: src/key.cpp =======================================================
// AGENT: PURPOSE    — File extension key/legend display
...
void FileKeyEntry::draw() {
    ...
    r.begin(GL_QUADS);
        ...
        r.vertex(pos.x, pos.y + height);
        r.vertex(pos.x + width, pos.y + height);
        r.vertex(pos.x + width, pos.y);
    r.end();

    r.popModelView();

    // Gradient quad - darker on left, brighter on right
    r.begin(GL_QUADS);
        r.color(colour.x * 0.5f, colour.y * 0.5f, colour.z * 0.5f, alpha);
        r.vertex(pos.x, pos.y);
        r.vertex(pos.x, pos.y + height);
        r.color(colour.x, colour.y, colour.z, alpha);
        r.vertex(pos.x + width, pos.y + height);
        r.vertex(pos.x + width, pos.y);
    r.end();
