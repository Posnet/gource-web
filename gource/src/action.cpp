// === File: src/action.cpp ====================================================
// AGENT: PURPOSE    — User action visualization (create/modify/delete beams)
...
void RAction::draw(float dt) {
    ...
    r.begin(GL_QUADS);
        ...
        r.vertex(src.x - offset_src.x, src.y - offset_src.y);
        r.texcoord(0.0f, 1.0f);
        r.vertex(src.x + offset_src.x, src.y + offset_src.y);

        r.color(col1);
        r.texcoord(1.0f, 1.0f);
        r.vertex(dest.x + offset.x, dest.y + offset.y);
